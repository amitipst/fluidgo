...
> for architecture state, feature status, known issues, and roadmap â€” update
...
changes. Don't delete old session log entries â€” this is a history, not just
a snapshot.
...
- **Stack:** React 18 PWA (Vite+TS) Â· FastAPI Â· PostgreSQL 15 Â· SQLAlchemy 2.0 async Â· Ollama (phi3:mini) Â· Docker Compose Â· Nginx reverse proxy
- **Deployed:** AWS EC2 (Mumbai/ap-south-1), IP `65.2.205.77`, app dir `/opt/fluidgo/app`
- **Repo:** `amitipst/fluidgo` (GitHub), branch `main`
- **Migrations:** Alembic, current head **0019** (see Â§4)
- **Deploy pattern:** local edit (Desktop Commander) â†’ `git push` â†’ on EC2: `bash <(curl -fsSL https://raw.githubusercontent.com/amitipst/fluidgo/main/update.sh)` â€” builds both images, runs migrations, restarts, runs smoke tests automatically.


## 2. Role Hierarchy (current, as of 0019)

```
CEO / super_admin  (level 99/50) â€” all businesses, all regions
  â””â”€â”€ COO           (level 45)  â€” all businesses (not scoped to one `business`)
        â””â”€â”€ Business Head        (level 40, scope=business) â€” ALL regions within ONE business line
              â”‚   ("BU" = business line: fluidpro/fluidprint/floxtax/hooks â€” NOT a region)
              â””â”€â”€ Regional Manager (level 30, scope=region) â€” ONE region within ONE business
                    â”‚   ("bu_head" = deprecated old name for this role, kept as alias only)
                    â””â”€â”€ Manager      (level 20, scope=team) â€” FULL reporting chain via manager_id,
                    â”‚                  any depth (recursive) â€” not just direct reports
                    â”‚     â”œâ”€â”€ Sales Rep / Inside Sales / Pre-Sales (level 10) â€” own data only
                    â”‚     â””â”€â”€ Service Delivery Manager (level 20, scope=team) â€” own reportees +
                    â”‚                  Service Delivery FGA/DOR track (separate from Sales/PreSales)
HR       (level 25, scope=hr)      â€” all users' FGA scores, no sales data
Finance  (level 25, scope=finance) â€” approved FGA export only
```

**Dual role (any level):** anyone can ALSO be personally set as someone's
`manager_id` via Team page â†’ "Reports to (Manager)", independent of their
...
primary role already grants â€” works everywhere (DSR approval, targets, FGA,
...
**Assignable via Team page today:** `rep` Â· `inside_sales` Â· `pre_sales` Â·
`manager` Â· `service_delivery_manager` Â· `regional_manager` Â· `business_head`
Â· `coo` Â· `hr` Â· `finance` Â· `ceo` Â· `super_admin`. (`bu_head` still works if
...
Config-driven â€” templates and weights live in `scoring_templates` /
`scoring_parameters`, never hardcoded. Two calc types:

- **`calc_type='pct'`** â€” value is a straight 0-100 achievement %; contribution
  = value Ã— (weight/100). Used by Sales (40/25/20/15) and PreSales (35/35/15/15).
- **`calc_type='tiered'`** â€” value is looked up against a `tiers` JSON list of
  bands to find a MULTIPLIER; contribution = weight Ã— multiplier (can exceed
  the parameter's base weight for strong performance, or hit 0 for weak).
  Used by Service Delivery FGA â€” validated to reproduce the source Excel
  exactly (112.5 total, all 6 KRAs match).

**`manual.*` metric_source** â€” no auto-calculator; value comes from
...
**Scoring Admin (`/scoring-admin`)** â€” any number of parameters (not fixed to
...
has an enforced FK to `org_roles.role_key` â€” creating a template with a
...
â†’ Manager reviews â†’ HR reviews/overrides â†’ VP approves â†’ Finance exports CSV.
**Service Delivery FGA has no approval workflow yet** â€” direct entry only.
...
| **0019** | scoring_results_unique | De-dupes + adds missing UNIQUE constraint on `scoring_results(user_id, template_id, period)` â€” fixes a real FGA-freeze 500 bug, see Â§6 |
...
| Area | Status | Notes |
|---|---|---|
| Revenue Intelligence (targets, quarterly/FY editor) | âœ… Live | Q1-Q4 grid, auto FY total, rollover pre-fill with growth% |
| RBAC â€” Regional Manager, dual-role, recursive hierarchy | âœ… Live | See Â§2 |
| Sales FGA / DSR | âœ… Live (UAT in progress per Amit) | |
| PreSales FGA / DSR | âœ… Live (believed UAT-ready, not independently re-verified this session) | |
| Service Delivery Manager â€” FGA + DOR | âœ… Live, ðŸ”¶ UAT blocked until org_roles fix + template reseed deployed (see Â§6) | |
| Reports section (CSV/PDF export) | âŒ Not started | Discussed, not built â€” see roadmap |
| CSG Phase 1 â€” Account entity, hunting/farming, deliveryâ†’sales signal | âœ… Live | `POST /api/dor/{id}/flag-opportunity` |
| CSG Phase 2-6 (Meetings/MOM/SIP/Timeline/AI engines/integrations) | ðŸ“‹ Roadmap only | See Linear doc "Epic 9 â€“ CSG Roadmap & Phasing" (project doc, not an issue â€” plan limit) |
| Auto session logoff (idle timeout) | âœ… Live | 30 min idle â†’ warning banner (60s countdown) â†’ auto-logout. Tunable in `useIdleLogout.ts` |
| Reports/README/architecture docs | âœ… Updated 2026-07-11 | This file + README.md |


## 6. Known Issues

### ðŸ”´ Open â€” needs your input or a repro
- **Team page crash on "Add Member" with a new role.** Reported 2026-07-11
  trying to onboard Hemant as Service Delivery Manager â€” full-page React
  ...
  empty DSR list â€” confirmed safe, returns 0.0 not a crash), and the auth
  refresh interceptor â€” found no bug via static review. The 401 pattern
  across 4 unrelated endpoints strongly suggests the session had already
  gone stale (token expiry) rather than a data-shape crash â€” auto-logout
  (Â§5) should help going forward, but the ORIGINAL crash cause is still
  ...
  Cannot read properties of undefined...") and paste that â€” the stack trace
  alone (minified function names) isn't enough to pinpoint it.**
- **Service Delivery FGA approval workflow** â€” not built. Direct-entry only
  right now. Needs a decision (see Â§3) before UAT if approval is required.
- **Reports section (CSV/PDF export)** â€” discussed at length, not built yet.

### âœ… Fixed this session (2026-07-11)
- **`org_roles` FK violation** â€” `scoring_templates.role_key` has an enforced
  ...
  one. Fixed two ways: (1) `seed_service_delivery.py` â€” new idempotent,
  ...
  the UI.
- **FGA freeze 500** â€” `scoring_results` only ever had an INDEX on
  ...
  `.scalar_one_or_none()` throws `MultipleResultsFound` â€” a real 500.
...
### âœ… Fixed in earlier sessions (carried over from README, still accurate)
- `/dsr/history` 500 â€” tz-naive/aware datetime comparison in `_edit_lock_state()`.
- `smoke_test.py` wrong port + 4 deactivated test accounts (reactivation
  script exists at `backend/reactivate_smoke_accounts.py` â€” **not run**,
  Amit chose to leave those accounts deactivated, 2026-07-11).
- Dual-hat role mapping (business_head personally managing a team) â€” now
  superseded by the fully recursive hierarchy in Â§2.


## 7. Roadmap (near â†’ far)

1. **Immediate:** deploy today's fixes, run `seed_service_delivery.py`,
   retry onboarding Hemant, retry FGA freeze, retry "+ New Template" â€” confirm
   all three are actually fixed against production, not just reasoned about.
2. **Reports section** â€” CSV/PDF export with date-range selector (DSR,
   Revenue, OB, Pipeline, Opportunities, Meetings). Designed, not built.
3. **CSG Phase 2** â€” Meeting Management + AI MOM Generator. Foundational
   data-capture layer; nothing in Phase 4/5 has data to analyze until this
   exists. See Linear project doc "Epic 9 â€“ CSG Roadmap & Phasing" for full
   phasing and the reasoning behind the order.
4. **CSG Phase 3** â€” Service Improvement Plans (SIP) + Customer Timeline.
5. **CSG Phase 4-5** â€” AI Customer Intelligence (health/risk scoring), AI
   Opportunity Engine (auto-surfaced expansion signals) â€” supersedes Phase
   1's manual "Flag as Opportunity" once there's enough meeting data to
   automate it.
6. **CSG Phase 6 (deferred)** â€” external integrations (ServiceDesk Plus,
   Site24x7, Seceon, Teams, Outlook), matching the existing WEP-42/43
   deferred-integration pattern.
7. **Linear plan** â€” upgrade or archive old issues; convert the CSG roadmap
...
editor, new-template UI) â€” validated against the source Excel exactly
(112.5 total, all 6 KRAs match). Regional Manager role (fixed the "BU Head"
mislabeling â€” BU = business line, not region). Dual-role RBAC (personal
...
split, Service Delivery â†’ Sales opportunity flagging). Fixed two real
production bugs (`org_roles` FK violation, `scoring_results` missing unique
constraint â€” see Â§6). Built auto session logoff (30 min idle timeout).
Created this tracker (Linear free-tier issue cap hit mid-session).

**Not resolved:** Team page crash on member creation â€” see Â§6 Open Issues.
Needs the actual browser console error text (not just the stack trace) next
time it happens.

**Migrations added:** 0017, 0018, 0019 â€” all additive, no destructive changes.

### 2026-07-13 (this session)
Built: Pipeline update history. Today's Update / Next Step on a deal were
overwritten in place on every Save â€” no trail of what a rep reported over
time. Added `pipeline_updates` append-only table (migration 0020, soft
references, no FK constraint â€” same pattern as `account_id`/
...
(`py_compile` on the three backend files, `tsc --noEmit` on the frontend â€”
both clean) and pushed to `main` (commit `89e876e`); not yet deployed to
EC2 or run against a live DB. Logged on WEP-37 (Epic 2 â€“ Opportunity
Intelligence) instead of a new sub-issue â€” workspace is over Linear's free
issue limit, update-in-place still works.

**Not resolved / next step:** this is the foundation, not the full ask â€”
...
**Migrations added:** 0020 â€” additive, no destructive changes.

---

### 2026-07-14 (this session)
Built: forced password change security hardening. Audited existing auth
first â€” bcrypt hashing, mandatory JWT_SECRET, short-lived access tokens,
...
0021, backfilled true for all existing rows â€” nobody grandfathered out).
Enforced server-side in `deps.get_current_user` via a path allowlist, not
just a frontend redirect â€” a still-valid access token cannot be used for
...
password server-side â€” the actor never chooses one â€” returned once,
...
**Important operational note â€” every existing UAT user, including Amit,
...
is a real behavior change on a live UAT with real reps â€” worth a heads-up
to the pilot team before/immediately after deploying, not a silent surprise.

**Migrations added:** 0021 â€” additive (two nullable/defaulted columns), no
...
all, since "My DSR Log" is `fieldOnly` and hidden for them â€” added a
...
the previously-pending AI-truncation/deal-stage fixes â€” all three (AI
...
- **Stall detection** (pure SQL, no LLM): `GET /pipeline` now returns
  ...
  stage (cold/warm/hot/qualification) â€” `on_hold` is an intentional pause
  and closed_* are terminal, so both are excluded. Surfaced as a
  ðŸŒ Stalled badge on the Pipeline card.
- **AI momentum check** (on-demand, rep-triggered): new
  ...
  (same as `generate_deal_postmortem`), asking for one short verdict â€”
  moving forward / stalled / going in circles â€” with supporting evidence.
...
on the frontend â€” both clean); not yet deployed to EC2 or run against a
live DB.

**Migrations added:** 0022 â€” additive (two nullable columns on `pipeline`),
no destructive changes.

### 2026-07-14, later same day â€” production regression + 3 more fixes
...
- **DSR reject not clearing the pending list.** `approve_dsr`'s reject
  branch set `approval_status` back to `"submitted"` â€” the exact status
  `team/pending` filters on â€” so a rejected DSR immediately reappeared in
  ...
  param (submitted/approved/rejected/all, default submitted â€” existing
  callers unaffected). `DSRHistory.tsx` adds Pending/Approved/Rejected
  tabs in Team Approval view. Commit `cb8d835`.
- **DOR/DMR approval â€” did not exist at all.** Confirmed `dor.py` had no
  `approval_status`, no approve/reject, no edit-lock â€” the model docstring
  ...
  aggregated monthly rollup) â€” added a "pending DORs awaiting review"
...
- **Team meetings showing only 1 for a Business Head â€” investigated, not
...
**Migrations added:** 0023 â€” additive (four nullable/defaulted columns on
`dor_daily`), no destructive changes.

### 2026-07-14, same day â€” HR role investigation + scheme winner validation
...
- **FGA Approval was a nav bug, not missing functionality.** The backend
  was already fully correct for HR â€” `resolve_visible_user_ids` explicitly
  grants HR org-wide visibility ("FGA audit only"), and `fga_approval.py`'s
  HR review step (`pending_hr` â†’ approve/override/dispute â†’ `pending_vp`)
  works exactly as designed. The bug: `Layout.tsx`'s Management section was
  gated on `(canSeeTeam || canSeeRevenue)`, and HR is in neither list â€”
...
- **Scheme winner validation genuinely didn't exist.** `incentives.py`'s
  ...
  progress view â€” nothing was ever persisted, and `PointsLedger` was never
  ...
  a status machine (`pending_hr` â†’ `approved`/`rejected`, plus a `paid`
  ...
  /mark-paid`. Frontend: "ðŸ” Detect Winners" button on manager scheme
...
**Migrations added:** 0024 â€” additive (new `scheme_winners` table), no
destructive changes.

### 2026-07-14, same day â€” HR nav trim + read-only Activity Logs
...
needs to see rigor scores + logs for DSR/DMR(pre-sales)/DOR â€” read-only,
no approve/reject.

- `Layout.tsx`: those five nav items were already tagged `salesOnly` and
  excluded for `isSDM`, but not for HR â€” extended the same filter to HR.
...
- New `ActivityLogs.tsx`: read-only page, three tabs (Sales DSR / Pre-Sales
  DMR / Service Delivery DOR â€” "DMR" is Amit's term for what the backend
  ...
  Team.tsx (which have approve/reject/manage-team actions baked in) â€” this
  ...
  level 25) â€” this page just renders them without the action affordances.
...
## 2026-07-14 â€” HR governance dashboard, SDM FGA inclusion, FGA-exempt flag (commit `df0041f`)
...
- **Root cause of (b)**: `fga_approval.py`'s `freeze_period()` role filter
  was `["rep","inside_sales","pre_sales","manager"]` â€” `service_delivery_manager`
...
- **Root cause of (a)**: `Dashboard.tsx`'s "Today's DSR" card was gated
  `!isBU && !isSDM` â€” HR is neither, so it rendered by accident. Also the
...
- **New `GET /fga-approval/bu-overview`** (HR/Finance/COO/CEO/super_admin
  only): headcount, FGA submission coverage %, avg score â€” grouped by
...
- **New `fga_exempt` boolean on `User`** (migration `0025`): freeze skips
  ...
  lives in Team.tsx's edit panel, surfaced as a "ðŸš« FGA N/A" badge.
- **orgLabel fix**: `hr`/`finance`/`coo` now show "All Regions Â· All
  Businesses" (previously fell through to a region/business string that
  didn't reflect their actual org-wide scope â€” only `ceo`/`super_admin`
  got this before).
- **Company-wide role onboarding**: Team.tsx's create/edit forms now hide
  ...
  them for these scopes â€” this was a UX-only gap, not a permissions bug).

Scope note: "Service operations" in Amit's request is treated as the same
role as "Service Delivery" (`service_delivery_manager`) â€” the only such
...
which they're already allowlisted for) â€” flagged as a reasonable next
step, not done.
...
to EC2 â€” needs `alembic upgrade head` on next deploy.

## 2026-07-15 â€” Ishaant/Modassir dashboard zeros investigation + soft-delete/archive (migration `0026`)
...
DB/SSH access from this side â€” Amit ran them):
...
- Confirmed via direct query: both users have a single account each (no
  ...
  intact and tied to the correct current user_id â€” Ishaant 32 calls/4
  ...
  exactly when run directly against the DB â€” so the query logic itself,
  as it exists in this repo, is correct.
- Found along the way: Ishaant has `manager_id=None` â€” no manager
...
- **Root cause, most likely**: `git log -1` on EC2 showed the repo pulled
  ...
  earlier** â€” i.e. `git pull` had happened but `docker compose build`
  ...
  hard-refresh to confirm. **Not yet confirmed fixed as of this entry** â€”
...
`opportunity_repo.list_opportunities` â€” every deal in the table shows up
...
hide-only) â€” chose soft-delete/archive.

- migration `0026`: `archived` (bool, default false) / `archived_at` /
  `archived_by` added to `pipeline`. Deals never hard-delete â€” they feed
  revenue/win-loss/FGA history â€” archiving just excludes them from every
...
- `pipeline.py`: `POST /{deal_id}/archive` and `/unarchive`, gated to
  ...
  **every** scope, not just "all" â€” `include_archived=true` opts back in.
...
- `Pipeline.tsx` / `Opportunities.tsx`: Archive/Unarchive button
  ...
  "ðŸ—„ï¸ Archived" badge on archived cards, and a "Show archived" checkbox
  for admins.

Verified: `python -m py_compile` and `tsc --noEmit` both clean. Not yet
deployed â€” bundled into the same rebuild as the stale-deploy fix above.

---

*This file supersedes README.md's "Recent Progress" and "Known Issues"
sections going forward â€” check here first. README.md stays the quick-start
reference; this file is the detailed, chronological record.*

## 2026-07-16 â€” End-to-end review (Claude/Rambo Commander session)
...
(`725534b`, 2 commits past tag `v1.0.5`) â€” perfectly in sync, nothing to
...
**Smoke suite was reporting 53% (16/30) â€” root-caused, not a real app
...
1. `backend/smoke_test.py` predates the `v1.0.5` mandatory
   ...
   every role â€” DSR submit, Meetings, Leads, Pipeline, Opportunities, FGA
...
2. Local docker-compose DB was on alembic `0025`; code expects `0026`
   ...
   soft-delete work in the 2026-07-15 session above) â€” every Pipeline/
   ...
   confirmed fixed" on EC2** â€” worth explicitly re-checking `alembic
...
on `require_level(20)` â€” the caller's own tier â€” never checked that the
...
(see repo history) â€” now actually closed.

**Still open, unconfirmed (carried over, no action taken):**
- Team page crash on "Add Member" with a new role (Â§6) â€” still needs the
  actual browser console error text next repro.
- Service Delivery FGA approval workflow â€” decision still pending.
- Reports section (CSV/PDF export) â€” not started.

**Committed locally, not pushed** (`b22eb3f`, on `main`) â€” held for Amit's
...
not gitignored) from the working tree â€” no commit needed, it was never
tracked.
## 2026-08-04 â€” R0 kickoff: consolidate 4 undeployed branches (Claude/Rambo Commander session, local dev on blr-dsk-amits)
...
an "AI-Native Services Intelligence Platform" â€” Sales â†’ Pre-sales â†’
Delivery â†’ Governance â†’ Customer Success â†’ Renewals â†’ Executive
...
(stabilize) â†’ R1 (primitives + DSRâ†”Meeting link) â†’ R2 (Customer Voice +
Executive digest) â†’ R3 (Governance/Escalation consolidation) â†’ R4
(Delivery Intelligence, build-vs-buy decision required) â†’ R5 (Practice
Mgmt + Knowledge Intelligence) â†’ R6 (Farming Intelligence + single AI
...
```
main (725534b)
 â””â”€ feature/dsr-backfill-and-seed-cleanup   (migration 0027)
     â””â”€ feature/feedback-and-help-guide      (migration 0028)
         â””â”€ feature/seed-data-pipeline-fix    (migration 0029)
             â””â”€ feature/compliance-export-governance (migration 0030)
```

`develop` was still sitting at `725534b` (unchanged since the 2026-07-16
end-to-end review). Fast-forwarded it straight to
`origin/feature/compliance-export-governance` (`b8180b2`) â€” a true
...
continuously for 6 days prior to this session â€” nginx:80, frontend:3002,
...
applying `0027 â†’ 0030` cleanly.

**Verification results:**
- `pytest tests/`: 24 passed, 7 failed â€” the 7 are the pre-existing
...
- `vertical_slice_test.py`: still broken as previously documented â€” stale
...
- `smoke_test.py --base http://nginx` (had to override the default
  ...
  - `manager@fluidpro.in` / `inside@fluidpro.in` logins â†’ 401. Root
    ...
    EC2 DB and in an **unpushed local commit on `main`** â€” it never made
  ...
  - `Dashboard: calls > 0` / `avg_rigor > 0` for `danish@fluidpro.in` â†’
    ...
    `is_seed=true` â€” **this is the exact tradeoff migration 0027's own
...
- Frontend `tsc --noEmit`: clean. `npm run build`: clean (PWA bundle
  generated, main chunk ~294 KB gzipped â€” the existing >500KB chunk-size
  warning is pre-existing, not introduced here).

**Pushed:** `develop` â†’ `origin/develop` (`725534b..b8180b2`, fast-forward).
**Opened:** [PR #1](https://github.com/amitipst/fluidgo/pull/1),
`develop` â†’ `main`, with the verification results above in the PR body.
**Deliberately not merged** â€” the existing auto-deploy-to-EC2 pipeline on
...
1. GitHub Actions billing lock â€” unresolved status not re-checked this
   session; still assumed blocking until Amit confirms otherwise.
2. Branch protection / required-status-checks on `main` â€” not yet applied,
   recommended before PR #1 or anything after it merges.
3. `vertical_slice_test.py` repair â€” still stale, still open.
4. The 2 unpushed commits on local `main` (`b22eb3f` scope-leak fix,
   `12582bc` docs) from the 2026-07-16 session â€” never reconciled with
   `develop`/`origin`; worth deciding whether to cherry-pick the
   scope-leak fix forward.
5. New UAT instance provisioning â€” Amit-owned, not started this session.
6. Version tag: `v1.1.0` already exists at the older `725534b` commit
   ...
   branches above) â€” recommend tagging the consolidated state `v1.2.0`
...
DSRâ†”Meeting structural-link gap (finding #3, open since 2026-07-21).

## 2026-08-04 (same day, continued) â€” Branch protection enabled on `main` and `develop`
...
included full branch protection rules on public repositories â€” no
...
doesn't apply here â€” this is a public repo under a personal account.)

Applied identically to both `main` and `develop` via `gh api PUT
.../branches/{branch}/protection`:
- Pull request required before merging â€” **no direct pushes, including
...
- `required_approving_review_count: 0` â€” deliberate: a PR object is
  still mandatory, but no second reviewer is required. GitHub does not
  allow a PR author to approve their own PR, so requiring â‰¥1 approval
...
- **No required status checks yet, on either branch** â€” deliberate, not
...
This PR (`chore/branch-protection-r0` â†’ `develop`) is itself the first
PR merged under the new rule â€” recording the policy change through the
...
(`develop`â†’`main`, the 4-branch consolidation) â€” all unchanged from the
