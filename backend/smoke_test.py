...
def api_get(path, token="", expected=200):
...
def api_post(path, body, token="", expected=200):
    headers = {"Authorization": f"Bearer {token}"} if token else {}
    try:
...
if ok:
    check("Health returns status:ok", r.json().get("status") == "ok", f"got: {r.json()}")
...
if nginx_url:
    ...
    check("Frontend serves HTML", True, "Verified via browser (set NGINX_URL to test)")
...
if tok_rep:
    r, ok = api_post("/dsr", {
        ...
        "visits": 2, "calls": 5, "followups": 10, "new_leads": 1, "proposals": 1
    }, tok_rep)
...
if tok_ps:
    ...
    if ok:
        check("Pre-Sales: dsr_type = presales", r.json().get("dsr_type") == "presales")
...
for path, label in BLOCKED:
    ...
    check(label, r is not None and r.status_code == 403, f"got HTTP {r.status_code if r else 'N/A'}")

r, _ = api_post("/fga/freeze", {"period": "2026-07"}, tok_rep)
check("Rep blocked from FGA freeze", r is not None and r.status_code == 403,
...
if tok_bu:
    ...
    if ok:
        d = r.json()
        check("Dashboard: calls > 0", d.get("total_calls", 0) > 0, f"calls={d.get('total_calls')}")
        check("Dashboard: avg_rigor > 0", d.get("avg_rigor", 0) > 0, f"rigor={d.get('avg_rigor')}")

    r, ok = api_get("/analytics/revenue?period=2026-05", tok_bu)
    check("BU Head: revenue analytics", ok)
    if ok:
        d = r.json()
        check("Revenue: target set",   d.get("target", 0) > 0, f"target=Rs{d.get('target')}")
...
if tok_bu:
    ...
    if ok:
        ...
        check("Sales FGA template exists",
              ...
              f"templates: {[t.get('role_key') for t in templates]}")
